...
#SBATCH --output=profiler_stdout.txt
#SBATCH --error=profiler_stderr.txt

tag=ablation_pretrain_token_profiler
...
fairseq-train /mnt/data/siqiouyang/datasets/must-c-v1.0 \
    ...
    --contrastive-weight 1.0 0.0 --contrastive-temperature 0.05 0.05 --contrastive-seqlen-type none --contrastive-level sentence \
    \
    --update-freq $(expr 16 / $num_gpus) --max-update 100 \
    \
    ...
    --best-checkpoint-metric contrastive_loss --enable-profiler