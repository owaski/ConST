...
fairseq-train /mnt/data/siqiouyang/datasets/must-c-v1.0 \
    ...
    --train-subset train_st_${TGT_LANG} --valid-subset dev_st_${TGT_LANG} \
    ...
    --contrastive-weight 1.0 --contrastive-temperature 0.02 --contrastive-seqlen-type none --contrastive-level token \
    \
    ...
    \
    --all-gather-list-size 32768 --use-pretrained-mfa 
    # --reset-optimizer --reset-dataloader 
    # --external-parallel-mt-data extra_mt/bin/ --text-data-sample-ratio 0.25


# sbatch --nodes=1 --ntasks=1 --cpus-per-task=24 --mem=64GB --gpus=2 --time=1-0:0:0 --mail-type=all --mail-user=siqiouyang@ucsb.edu --output=runs/main_ende_token_0.02_stdout.txt --error=runs/main_ende_token_0.02_stderr.txt ConST/scripts/main/ende_token_0.02_cont.sh